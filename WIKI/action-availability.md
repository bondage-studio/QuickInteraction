...
`ActivityAllowedForGroup(character, group)` 的回傳值已經包含 BC 對角色狀態、權限、互動方向、道具需求及部位可用性的判定。取得這份清單後，不應再逐動作呼叫或模擬同一套可用性檢查。

目前採用以下流程：

1. 對代表部位呼叫一次 `ActivityAllowedForGroup`。
2. 直接採用 BC 回傳的權威清單。
3. 只做插件自身的隱藏、ECHO 抑制、名稱清理與去重。
4. 少數強制顯示動作另外補入，並快取判定結果。
5. 只有 BC API 缺失或拋出錯誤時才使用全量 fallback；權威清單為空代表真的沒有可用動作。

舊文件中提到的 `actionExecutable`／`allowedNamesFor` 二次驗證已移除。該作法會讓動作較多的部位產生明顯延遲，且與 BC 原生過濾重複。
