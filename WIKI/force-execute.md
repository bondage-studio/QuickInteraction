...
```js
var FORCE_AVAILABLE_ACTION_NAMES = ['流出液體','流出液体','失禁','鑽進懷裡','钻进怀里','抱入懷中','抱入怀中','躺上去','拉上床','拉到床上'];
```
...
强制只保证 **QiAct 侧不再拦截、且把动作发上线**。echo 自己的 `run` 处理器仍会做**它自己**的运行期检查（`躺上去` / `拉上床` 查床、`钻进怀里`/`抱入懷中` 查「贴贴」资产）。若 echo 的 `run` 因真实道具/状态缺失而空转，消息照发但 echo 的视觉效果可能不生效——那属于 echo 服务端 hook 的逻辑，QiAct 不去侵入覆盖（否则脆弱且易随 echo 更新失效）。
