    ...
    (function() {
      ...
      function relationshipMatches(character, group) {
        ...
        if (group === "lover") {
          ...
          if (Array.isArray(lovers) && lovers.some(function(entry) {
            return parseInt(entry && (entry.MemberNumber || entry.Member), 10) === id;
          })) return true;
          try {
            var afc = window.Liko && window.Liko.AFC;
            return !!(afc && typeof afc.getLovers === "function" && afc.getLovers().some(function(entry) {
              return parseInt(entry && (entry.memberNumber || entry.MemberNumber), 10) === id;
            }));
          } catch (e) {
            return false;
          }
        ...
        if (group === "sub") {
          ...
          var roomCharacter = typeof ChatRoomCharacter !== "undefined" && (ChatRoomCharacter || []).find(function(c) {
            return c && parseInt(c.MemberNumber, 10) === id;
          });
      ...
      function isActionAllowedCharacter(character) {
        ...
        return state.actionAllowGroups.some(function(group) {
          return relationshipMatches(character, group);
        });
      }
      ...
      BODY_PARTS.forEach(function(part) {
        ...
        Object.defineProperty(part, "label", {
          ...
          enumerable: true,
          get: function() {
            return QiActT("part." + part.group, void 0) || staticLabel;
          }
        });
      });
      ...
      function collectBlockedAction(group, name) {
        var item = state.blockedActions.find(function(entry) {
          return entry.name === name;
        });
        if (!item) {
          item = { name, group: canonicalPartGroup(group), selfBlocked: false, otherBlocked: false, selfPrevious: 2, otherPrevious: 2 };
      ...
      function toggleBlockCaptureMode() {
        ...
        if (state.blockCaptureActive && state.favModeActive) {
          state.favModeActive = false;
          updateFavButtonVisual();
        }
        updateBlockUiVisual();
      ...
      function updateBlockedActionsPanel() {
        ...
        var html = '<label class="xsact-block-master"><span><strong>' + QiActT("block.filtering") + "</strong><small>" + QiActT("block.filtering_hint") + '</small></span><span class="xsact-switch"><input type="checkbox" id="xsact-block-filtering"' + (state.blockFilteringEnabled ? " checked" : "") + '><span class="xsact-switch-track"></span></span></label><div class="xsact-block-table"><div class="xsact-block-head"><span>' + QiActT("block.part") + "</span><span>" + QiActT("block.action_name") + "</span><span>" + QiActT("block.on_self") + "</span><span>" + QiActT("block.on_others") + "</span><span>" + QiActT("block.delete") + "</span></div>";
        state.blockedActions.forEach(function(item, index) {
          html += '<div class="xsact-block-row" data-index="' + index + '"><span class="xsact-block-part">' + escapeHtml(QiActT("part." + item.group)) + '</span><span class="xsact-block-name">' + escapeHtml(getActivityLabel(item.name, item.group)) + '</span><label class="xsact-switch"><input type="checkbox" data-direction="self"' + (item.selfBlocked ? " checked" : "") + '><span class="xsact-switch-track"></span></label><label class="xsact-switch"><input type="checkbox" data-direction="other"' + (item.otherBlocked ? " checked" : "") + '><span class="xsact-switch-track"></span></label><button class="xsact-block-delete" title="' + QiActT("block.delete") + '" data-tooltip-type="danger">' + svgIcon("trash", 14) + "</button></div>";
        });
        if (!state.blockedActions.length) html += '<div class="xsact-qa-empty">' + QiActT("block.empty") + "</div>";
        listEl.innerHTML = html + "</div>";
        listEl.querySelector("#xsact-block-filtering").addEventListener("change", function(e) {
          setBlockFilteringEnabled(e.target.checked);
        });
      ...
      function setFloatingButtonVisible(on) {
        state.floatingButtonVisible = !!on;
        persist(S_FLOATING_BUTTON, state.floatingButtonVisible);
        if (!state.floatingButtonVisible && state.toggleBtnEl) {
          state.toggleBtnEl.remove();
      ...
      function updateToggleBtnStyle() {
        ...
        if (state.toggleBtnEl) {
          state.toggleBtnEl.classList.toggle("active", state.isActive);
          state.toggleBtnEl.title = tooltip;
        }
        if (state.chatToggleBtnEl) {
          state.chatToggleBtnEl.classList.toggle("active", state.isActive);
          state.chatToggleBtnEl.removeAttribute("title");
        }
      ...
      function drawToggleButton() {
        if (!state.chatToggleBtnEl || !document.documentElement.contains(state.chatToggleBtnEl)) registerChatRoomToggle();
        if (state.floatingButtonVisible && (!state.toggleBtnEl || !document.body.contains(state.toggleBtnEl))) {
          state.toggleBtnEl = null;
          createToggleButton();
        }
        if (state.toggleBtnEl) {
      ...
      function enterActionMode() {
        ...
        if (!/^(part|favorite|combo|custom|blocked)$/.test(savedMode) || savedMode === "blocked" && !state.blockActionsEnabled) savedMode = "part";
      ...
      function bodyGridTopologySignature(layout) {
        return (layout || []).map(function(entry) {
          ...
          if (isPlayer && !state.drawSelfWireframe) return null;
      ...
      function refreshBodyGrids(precomputedLayout) {
        ...
        layout.forEach(function(entry) {
          ...
          if (isPlayer && !state.drawSelfWireframe) return;
      ...
      function renderCharList() {
        ...
        if (chars.length === 0) {
          ...
          chars.forEach(function(c) {
            ...
            html += '<div class="xsact-char-popover-item' + (selected ? " selected" : "") + (isSelf ? " self" : "") + '" data-mn="' + c.MemberNumber + '"><span class="xsact-char-popover-name">' + escapeHtml(characterDisplayName(c) + " (" + c.MemberNumber + ")") + "</span>" + (isSelf ? '<span class="xsact-char-popover-self">' + QiActT("common.self") + "</span>" : "") + "</div>";
          });
          html += "</div>";
        }
      ...
      function updateComboPanel(charObj) {
        ...
        if (state.editingComboId) {
          ...
          html += '<div class="xsact-combo-field xsact-combo-delay"><label id="xsact-delay-label">' + QiActT("combo.delay_label", { n: curDelay }) + '</label><input type="range" id="xsact-combo-delay" min="50" max="2000" step="50" value="' + curDelay + '"></div>';
      ...
      function updateFavoritesPanel(charObj) {
        ...
        if (state.selfModeActive && typeof ChatRoomCharacter !== "undefined" && Array.isArray(ChatRoomCharacter)) {
          favoriteTarget = ChatRoomCharacter.find(function(c) {
            return c && c.IsPlayer && c.IsPlayer();
          }) || favoriteTarget;
        }
      ...
      function updateSettingsPanel() {
        ...
        function idChips(ids, kind) {
          return ids.map(function(id) {
            return '<button type="button" class="xsact-id-chip" data-list="' + kind + '" data-id="' + id + '" title="' + QiActT("settings.remove_id") + '">' + id + " ×</button>";
          }).join("");
        }
        ...
        function bindIdEditor(kind) {
          ...
          function commit() {
            ...
            if (kind === "allow") {
              state.actionAllowMembers = next;
              persist(S_ACTION_ALLOW_MEMBERS, next);
            } else {
              state.actionSkipMembers = next;
              persist(S_ACTION_SKIP_MEMBERS, next);
            }
            updateSettingsPanel();
          }
        ...
        bindIdEditor("allow");
        bindIdEditor("skip");
        listEl.querySelectorAll(".xsact-id-chip").forEach(function(chip) {
          chip.addEventListener("click", function() {
            var id = parseInt(chip.dataset.id, 10), kind = chip.dataset.list;
            if (kind === "allow") {
              state.actionAllowMembers = state.actionAllowMembers.filter(function(x) {
                return x !== id;
              });
              persist(S_ACTION_ALLOW_MEMBERS, state.actionAllowMembers);
            } else {
              state.actionSkipMembers = state.actionSkipMembers.filter(function(x) {
                return x !== id;
              });
        ...
        listEl.querySelectorAll("[data-allow-group]").forEach(function(box) {
          box.addEventListener("change", function() {
            state.actionAllowGroups = Array.from(listEl.querySelectorAll("[data-allow-group]:checked")).map(function(x) {
              return x.dataset.allowGroup;
            });
            persist(S_ACTION_ALLOW_GROUPS, state.actionAllowGroups);
          });
        });
        listEl.querySelectorAll('input[name="xsact-all-scope"]').forEach(function(radio) {
          radio.addEventListener("change", function() {
            if (radio.checked) {
              state.allTargetScope = radio.value;
              persist(S_ALL_TARGET_SCOPE, radio.value);
            }
          });
        });
        ...
        listEl.querySelector("#xsact-settings-chat").addEventListener("change", function(e) {
          setFloatingButtonVisible(e.target.checked);
        });
        ...
        listEl.querySelector("#xsact-settings-block-actions").addEventListener("change", function(e) {
          ...
          if (!state.blockActionsEnabled) {
            state.blockUiActive = false;
            state.blockCaptureActive = false;
          }
      ...
      function injectStyles() {
        ...
        css.textContent = [
          ...
          ".xsact-qa-mini-btn.on,#xsact-block-manager-btn.on{color:var(--xs-accent-text);border-color:var(--xs-accent);background:rgba(var(--xs-accent-rgb),.16);box-shadow:0 0 10px rgba(var(--xs-accent-rgb),.18);}",
          "#xsact-action-list.xsact-blocked-mode{display:flex;flex-direction:column;gap:8px}.xsact-block-master{display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%;box-sizing:border-box;padding:9px 10px;border:1px solid var(--xs-border);border-radius:8px;background:var(--xs-btn-bg);color:var(--xs-text)}.xsact-block-master strong{display:block;font-size:12px}.xsact-block-master small{display:block;margin-top:2px;font-size:10px;line-height:1.3;color:var(--xs-text-dim)}.xsact-block-table{display:flex;flex-direction:column;gap:6px;width:100%;min-width:0;box-sizing:border-box}.xsact-block-head,.xsact-block-row{display:grid;grid-template-columns:minmax(50px,.65fr) minmax(72px,1.35fr) 58px 58px 42px;width:100%;box-sizing:border-box;align-items:center;gap:5px;padding:7px 8px;border:1px solid var(--xs-border);border-radius:8px}.xsact-block-head{color:var(--xs-text-dim);font-size:10px;text-align:center;background:var(--xs-btn-bg)}.xsact-block-head span:nth-child(-n+2){text-align:left}.xsact-block-row{color:var(--xs-text);background:var(--xs-btn-bg)}.xsact-block-part,.xsact-block-name{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px}.xsact-block-part{color:var(--xs-text-dim);font-size:11px}.xsact-block-row .xsact-switch{justify-self:center}.xsact-block-delete{justify-self:center;width:30px;height:30px;display:flex;align-items:center;justify-content:center;border:1px solid var(--xs-border);border-radius:7px;background:transparent;color:var(--xs-text-dim);cursor:pointer}.xsact-block-delete:hover{color:#ff7b91;border-color:#ff5c7a;background:rgba(255,92,122,.12)}",
          ...
          ".xsact-settings-group{display:flex;flex-direction:column;gap:8px;padding:10px;border:1px solid var(--xs-border);border-radius:9px;background:var(--xs-btn-bg);}",
          ".xsact-settings-group-title{align-self:flex-start;padding:7px 10px;border:1px solid rgba(var(--xs-accent-rgb),.45);border-radius:8px;background:rgba(var(--xs-accent-rgb),.10);font-size:12px;font-weight:700;color:var(--xs-accent-text);}",
          ".xsact-id-editor{display:flex;gap:6px}.xsact-id-editor input{min-width:0;flex:1;background:var(--xs-panel-bg);color:var(--xs-text);border:1px solid var(--xs-border);border-radius:7px;padding:7px}.xsact-id-editor button{width:30px;height:30px;padding:0;border:1px solid var(--xs-border-strong);border-radius:7px;background:var(--xs-panel-bg);color:var(--xs-text-dim);cursor:pointer}.xsact-id-editor #xsact-settings-allow-add{border-color:rgba(var(--xs-accent-rgb),.6);border-radius:6px;background:rgba(var(--xs-accent-rgb),.12);color:var(--xs-accent-text)}",
          ".xsact-id-chips,.xsact-relation-options,.xsact-scope-options{display:flex;flex-wrap:wrap;gap:6px}.xsact-id-chip{padding:4px 7px;border:1px solid var(--xs-border-strong);border-radius:999px;background:var(--xs-panel-bg);color:var(--xs-text-dim);font-size:11px;cursor:pointer}.xsact-id-chip:hover{border-color:#FF5C5C;color:#FFB3B3}.xsact-scope-options label{display:flex;align-items:center;gap:4px;font-size:11px;color:var(--xs-text-dim)}",
          ".xsact-relation-options label{position:relative;display:inline-flex;cursor:pointer}.xsact-relation-options input{position:absolute;opacity:0;pointer-events:none}.xsact-relation-options span{padding:3px 9px;border:1px solid var(--xs-border-strong);border-radius:999px;background:var(--xs-panel-bg);color:var(--xs-text-dim);font-size:10px;font-weight:700;transition:all .15s}.xsact-relation-options label:hover span{filter:brightness(1.15)}",
          ".xsact-relation-owner input:checked+span{background:#2a0808;color:#ff8888;border-color:#801818}.xsact-relation-lover input:checked+span{background:#28082a;color:#ff9ae0;border-color:#801868}.xsact-relation-sub input:checked+span{background:#082018;color:#60e0b0;border-color:#106040}.xsact-relation-whitelist input:checked+span{background:#0d2a1a;color:#60d090;border-color:#208050}.xsact-relation-friend input:checked+span{background:#08102a;color:#88c8ff;border-color:#184880}",
          ".xsact-settings{font-size:calc(12px + 2pt)}.xsact-settings-row{font-size:calc(12px + 2pt)}.xsact-settings-row select,.xsact-settings-number input,.xsact-id-editor input{font-size:calc(12px + 2pt)}.xsact-settings-row strong,.xsact-settings-group-title{font-size:calc(12px + 2pt)}.xsact-settings-row small{font-size:calc(10px + 2pt)}.xsact-settings-number em,.xsact-id-chip{font-size:calc(11px + 2pt)}.xsact-relation-options span{font-size:calc(10px + 2pt)}",
          ".xsact-settings-group-title{align-self:flex-start;padding:0;border:0;border-radius:0;background:transparent;color:var(--xs-text);box-shadow:none}",
          ".xsact-id-editor button,.xsact-id-editor #xsact-settings-allow-add,.xsact-id-editor #xsact-settings-skip-add{width:34px;height:34px;padding:0;border:1px solid rgba(var(--xs-accent-rgb),.6);border-radius:6px;background:rgba(var(--xs-accent-rgb),.12);color:var(--xs-accent-text);font-size:calc(12px + 2pt)}",
          ".xsact-scope-options label{position:relative;display:inline-flex;cursor:pointer}.xsact-scope-options input{position:absolute;opacity:0;pointer-events:none}.xsact-scope-options span{display:flex;align-items:center;gap:6px;padding:7px 9px;border:1px solid var(--xs-border);border-radius:8px;background:var(--xs-btn-bg);color:var(--xs-text-dim);font-size:calc(11px + 2pt);font-weight:600;transition:background .15s,border-color .15s,color .15s,box-shadow .15s}.xsact-scope-options i{width:7px;height:7px;border-radius:50%;background:var(--xs-text-faint);border:1px solid var(--xs-border-strong)}.xsact-scope-options label:hover span{background:var(--xs-hover);border-color:var(--xs-border-strong);color:var(--xs-text)}.xsact-scope-options input:checked+span{color:var(--xs-accent-text);border-color:rgba(var(--xs-accent-rgb),.6);background:rgba(var(--xs-accent-rgb),.12);box-shadow:0 0 10px rgba(var(--xs-accent-rgb),.12)}.xsact-scope-options input:checked+span i{background:var(--xs-accent);border-color:var(--xs-accent);box-shadow:0 0 8px rgba(var(--xs-accent-rgb),.7)}",
          ...
          "  z-index:1000;overflow:hidden;contain:layout paint style;",
          ...
          ".xsact-tooltip{",
          "  position:fixed;z-index:1000010;max-width:240px;",
      ...
      async function main() {
        ...
        state.actionAllowGroups = (loadSetting(S_ACTION_ALLOW_GROUPS, []) || []).filter(function(group) {
          return ["owner", "lover", "sub", "whitelist", "friend"].indexOf(group) >= 0;
        });
