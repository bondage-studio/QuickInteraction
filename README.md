...
- **多语言界面**：支持繁中、简中、英文、日文、韩文、越南文、德文、法文、西班牙文、俄文、乌克兰文及自动侦测；缺失键自动回退英文，永不显示空白
...
翻译来源放在根目录 [`Translation`](./Translation)，每个语言使用独立 JSON 文件，键名必须与 `EN.json` 一致。新增语言时同时更新 `src/i18n/runtime.js` 的语言清单与原生名称；执行 `npm run build` 会检查必要语言、内嵌全部翻译并生成发布脚本。

日文、韩文、越南文、西班牙文等机器翻译初稿仍建议由母语使用者校对，尤其是动作与成人语境用词。
...
- 【翻譯架構】翻譯來源改為根目錄 `Translation`，每個語系一個 JSON，方便社群獨立校對與擴充
- 【新增】加入收藏分頁，可集中管理與使用所有收藏動作，並支援人物與部位篩選
- 【介面】設定集中至齒輪頁；新增公告按鈕、人物列表方向、互動格、聊天室按鈕收納與小酥動作包開關
- 【我的動作】重整編輯版面、部位家族高亮、手持動作偵測、佔位符插入與手機大型部位選擇視窗
- 【穩定性】完善熱移除／熱注入清理，修正按鈕狀態、面板溢出與停用後無法重新啟用等問題
